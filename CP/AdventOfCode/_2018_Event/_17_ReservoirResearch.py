...
with open(f"{cwd}/CP/AdventOfCode/_2018_Event/_17_ReservoirResearch.txt") as f:
    data = list(f)
...
for ln in data:
    ...
    if m:
        exploded.extend(
            (int(m.group(1)), y) for y in range(int(m.group(2)), 1 + int(m.group(3)))
        )
        continue
    m = re.match(r"y=(-?\d+), x=(-?\d+)\.\.(-?\d+)", ln)
    if m:
        exploded.extend(
            (x, int(m.group(1))) for x in range(int(m.group(2)), 1 + int(m.group(3)))
        )
        continue
