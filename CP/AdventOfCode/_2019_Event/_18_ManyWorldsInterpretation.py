...
def get_grid(part_b=False):
    ...
    with open(
        f"{cwd}/CP/AdventOfCode/_2019_Event/_18_ManyWorldsInterpretation.txt"
    ) as f:
            ...
            for k1, v1 in v0.items():
                if not k1 & path.collected_keys:
                    ...
                    if doors_in_way & path.collected_keys == doors_in_way:
                        ...
                        yield Path(
                            new_position, path.collected_keys + k1, path.length + dist
                        )
...
def find_smallest_path(grid, keys, doors, start_points, max_x, max_y):
    ...
    start_points_bits = int(
        np.bitwise_or.reduce(list(map(lambda x: 1 << x, start_points_nums)))
    )
