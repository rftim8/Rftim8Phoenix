...
def is_valid(image: List[List[Optional[Matrix[T]]]], size: int):
    for i, j in itertools.product(range(size), range(size)):
        ...
        if i and (other := image[i - 1][j]) and current[0] != other[-1]:
            ...
            j
            and (other := image[i][j - 1])
            and get_matrix_column(current, 0) != get_matrix_column(other, -1)
        ):
            return False
        elif (
            j != size - 1
            and (other := image[i][j + 1])
            and get_matrix_column(current, -1) != get_matrix_column(other, 0)
        ):
            return False
...
def try_tiles_in_image(
    image: List[List[Optional[Matrix[str]]]],
    size: int,
    i: int,
    j: int,
    tile_ids: MutableSequence[int],
    tiles: Dict[int, Matrix[str]],
):
    for eid in tile_ids:
...
def solve_two(data: str):
    ...
    for c_variation in generate_matrix_variations(corner_tile):
        ...
        if all(
            try_tiles_in_image(image, size, i, j, corner_edges, tiles)
            for i, j in ((1, 0), (0, 1))
        ):
            break
    ...
    for variation in generate_matrix_variations(full_image):
        ...
        for i, j in itertools.product(range(rows), range(columns)):
            ...
            for l, regex in enumerate(regexes):
                match = re.search(
                    regex, "".join(variation[i + l][j : j + regexes_width])
                )
                if not match:
                    start = None
                    break
...
print(
    solve_one(open(f"{cwd}/CP/AdventOfCode/_2020_Event/_20_JurassicJigsaw.txt").read())
)
print(
    solve_two(open(f"{cwd}/CP/AdventOfCode/_2020_Event/_20_JurassicJigsaw.txt").read())
)
